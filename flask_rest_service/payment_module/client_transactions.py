...
def Sorting(*args, **kwargs):
    ...
    main_query = mongo.db.checkout_transactions.find({'clientId': ObjectId( kwargs.get('current_user'))})
...
class CheckoutTransactions(Resource):
    @jwt_required
    def get(self, page): 
        ...
        main_query = mongo.db.checkout_transactions.find({'clientId': ObjectId(current_user)})
        total_records = main_query.count()
...
class ClientCaseTransactions(Resource):
    @jwt_required
    def get(self, caseId): 
        current_user = get_jwt_identity()
        user_details = mongo.db.users.find_one( { '_id': ObjectId(current_user) } )
