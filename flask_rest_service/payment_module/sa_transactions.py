    ...
    result = mongo.db.checkout_transactions.find( 
        {"$and": [  
                    { "$or": [ 
                                ...
                                { "paid_amount": { "$regex": f".*{kwargs.get('search_keyword')}.*"} } , 
...
def SearchandSorting(*args, **kwargs):
    transactions = []
    main_condition = {'clientId': ObjectId( kwargs.get('current_user'))}
...
def Filter(*args, **kwargs):
    ...
    result = mongo.db.checkout_transactions.find ( { "$or": query_list } )
    total_records = result.count()
    for transaction in result.sort("_id", -1).limit(kwargs.get('table_rows')).skip(kwargs.get('offset')):
        transactions.append(transaction)
    return transactions, total_records
...
def Sorting(*args, **kwargs):
    ...
    main_query = mongo.db.checkout_transactions.find()
...
class SACaseTransactions(Resource):
    @jwt_required
    def get(self, caseId): 
        current_user = get_jwt_identity()
        user_details = mongo.db.users.find_one( { '_id': ObjectId(current_user) } )
        transactions = []
        main_query = mongo.db.checkout_transactions.find({'caseId': ObjectId(caseId)})
