...
def SearchandFilterandSorting(*args, **kwargs):
    ...
    condition_one = {"forwardTo": { "$elemMatch" : {"$eq" : ObjectId(kwargs.get('id')) } } }
    ...
    main_condition = { "$or": [ {'client': ObjectId(kwargs.get('id'))}, {"$or": [{"$and": [condition_one, condition_two]}, {"serviceProvider": ObjectId(kwargs.get('id'))}]} ] } 
...
def InitialRecords(*args, **kwargs):
    ...
    # This is because the cases that are in progress cannot be seen by other service providers
    condition_two = {"$or" : [{"status": {"$ne": "On-progress"}}, {"status": {"$ne": "Completed"}}]}
    main_query = mongo.db.cases.find( { "$or": [ {'client': ObjectId(kwargs.get('id'))}, {"$or": [{"$and": [condition_one, condition_two]}, {"serviceProvider": ObjectId(kwargs.get('id'))}]} ] } )
...
# For all the People cases in super admin dashboard
class PeopleCases(Resource):
    @jwt_required
    def get(self, id, page):
        ...
        # condition to check if the user is listed in the forwardTo
        condition_one = {"forwardTo": { "$elemMatch" : {"$eq" : ObjectId(id) } } }
        # condition to check if the case that has the current user listed in the forwardTo has not been to progress
        # This is because the cases that are in progress cannot be seen by other service providers
        condition_two = {"$or" : [{"status": {"$ne": "On-progress"}}, {"status": {"$ne": "Completed"}}]}
        main_query = mongo.db.cases.find( { "$or": [ {'client': ObjectId(id)}, {"$or": [{"$and": [condition_one, condition_two]}, {"serviceProvider": ObjectId(id)}]} ] } )
